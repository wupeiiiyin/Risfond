...
import com.hyphenate.easeui.utils.ImeUtil;
...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    static final int ITEM_USER_CARD = 3;

    protected int[] itemStrings = {R.string.attach_picture, R.string.attach_take_pic/*, R.string.user_card*/};
    protected int[] itemdrawables = {R.drawable.ease_chat_image_selector, R.drawable.ease_chat_takepic_selector/*, R.drawable.ease_chat_takepic_selector*/};
    protected int[] itemIds = {ITEM_PICTURE, ITEM_TAKE_PICTURE/*, ITEM_USER_CARD*/};
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        EaseUI.getInstance().chattingId = toChatUsername;
        fromNickName = fragmentArgs.getString("fromNickName");
        fromNickUrl = fragmentArgs.getString("fromNickUrl");
        toNickName = fragmentArgs.getString("toNickName");
        toNickUrl = fragmentArgs.getString("toNickUrl");
    ...
    protected void initView() {
        ...
        inputMenu.setChatInputMenuListener(new ChatInputMenuListener() {
            ...
            public void onBigExpressionClicked(EaseEmojicon emojicon) {
                sendBigExpressionMessage(emojicon.getExpressioType(), emojicon.getName(), emojicon.getIdentityCode());
            }
        });
        ...
        ((EaseEmojiconMenu) inputMenu.getEmojiconMenu()).addEmojiconGroup(EmojiconAddGroupData.getData());
    }
    ...
    protected void setUpView() {
        ...
        titleBar.setLeftLayoutClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ImeUtil.hideSoftKeyboard(sendMessageET);
                onBackPressed();
            }
        });
            ...
            public void onBubbleLongClick(EMMessage message) {
                ...
                if (chatFragmentHelper != null) {
                    ...
                    if (message.getType() == EMMessage.Type.TXT) {
                        ...
                        if (type.equals("")) {//发送的是文本和表情
                            EaseCommonUtils.showCopy(context, ((EMTextMessageBody) message.getBody()).getMessage());
                        }
                    }
                }
            }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAMERA) { //拍照
                if (cameraFile != null && cameraFile.exists())
                    sendImageMessage(cameraFile.getAbsolutePath());
            } else if (requestCode == REQUEST_CODE_LOCAL) { //图片选择
                if (data != null) {
    ...
    public void onResume() {
        ...
        sendMessageET = (EditText) getView().findViewById(R.id.et_sendmessage);
        voiceBtn = (Button) getView().findViewById(R.id.btn_set_mode_voice);
        moreBtn = (Button) getView().findViewById(R.id.btn_more);
        emenjoyIV = (ImageView) getView().findViewById(R.id.iv_face_normal);

        /*addOnSoftKeyBoardVisibleListener((Activity) context, new OnSoftKeyBoardVisibleListener() {
            @Override
            public void onSoftKeyBoardVisible(boolean v) {
                if (v) {
                    ...
                    ImeUtil.showSoftKeyboard(sendMessageET);
                }
            }
        });*/
        sendMessageET.setOnTouchListener(new OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                //                listView.dispatchTouchEvent(MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_CANCEL, 0, 0, 0));
                listView.setSelection(listView.getAdapter().getCount() - 1);
        ...
        emenjoyIV.setOnTouchListener(new OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                //                listView.dispatchTouchEvent(MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_CANCEL, 0, 0, 0));
                listView.setSelection(listView.getAdapter().getCount() - 1);
        ...
        moreBtn.setOnTouchListener(new OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                //                listView.dispatchTouchEvent(MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_CANCEL, 0, 0, 0));
                listView.setSelection(listView.getAdapter().getCount() - 1);
    ...
    private Button voiceBtn, moreBtn;
    private ImageView emenjoyIV;
    ...
    protected void sendBigExpressionMessage(String expressioType, String name, String identityCode) {
        EMMessage message = EaseCommonUtils.createExpressionMessage(toChatUsername, expressioType, name, identityCode);
        sendMessage(message);
    }
        ...
        switch (type) {
            case TXT:
                if (forward_msg.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)) {
                    sendBigExpressionMessage(forward_msg.getStringAttribute(EaseConstant.MESSAGE_ATTR_EXPRESSION_ID, null),
