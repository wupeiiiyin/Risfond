...
public class SuccessCaseDetailActivity extends BaseActivity {
    ...
    public void init(Bundle savedInstanceState) {
        context = SuccessCaseDetailActivity.this;
        id = getIntent().getStringExtra("id");
        url = URLConstant.URL_CASE_DETAIL_H5 + "id=" + id ;
        initWebView();
        title = "案例详情";
        tvTitle.setText(title);
        mivTitleRight.setImageResource(R.mipmap.ficonshare);
        mllTitleSearch.setVisibility(View.VISIBLE);
    }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.ll_title_search) {
            if(js2evaluate != null){
                ShareUtil.showShare(context, "成功案例："+js2evaluate.getTitle(), js2evaluate.getDetails(), js2evaluate.getImg(), js2evaluate.getUrl());
            }else {
